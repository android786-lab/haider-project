...
const bookAppointment = async (req, res) => {
    try {
        ...
        const { data: docData, error: docErr } = await supabase
            .from('doctors').select('*, users(id, name, email, image)').eq('user_id', docId).single()
        ...
        const { error: apptErr } = await supabase.from('appointments').insert({
            ...
            slot_date: slotDate,
            slot_time: slotTime,
            patient_snapshot: userData,
            doctor_snapshot: { ...docData, user: docData.users },
            amount: docData.fees,
            status: 'payment_pending',
        })
        if (apptErr) throw apptErr

        await supabase.from('doctors').update({ slots_booked }).eq('user_id', docId)
...
const cancelAppointment = async (req, res) => {
    try {
        ...
        if (appointment.patient_id !== userId) return res.json({ success: false, message: 'Unauthorized action' })

        await supabase.from('appointments').update({ status: 'cancelled' }).eq('id', appointmentId)

        const { data: doctor, error: docErr } = await supabase
            .from('doctors').select('slots_booked').eq('user_id', appointment.doctor_id).single()
        if (docErr) throw docErr
        ...
        await supabase.from('doctors').update({ slots_booked }).eq('user_id', appointment.doctor_id)
...
const listAppointment = async (req, res) => {
    try {
        ...
        const { data: appointments, error } = await supabase
            .from('appointments').select('*').eq('patient_id', userId)
        if (error) throw error
...
const paymentSTRIPE = async (req, res) => {
    try {
        ...
        const { data: appointment, error } = await supabase
            .from('appointments').select('*').eq('id', appointmentId).single()
        if (error || !appointment || appointment.status === 'cancelled') {
...
const verifySTRIPE = async (req, res) => {
    try {
        ...
        if (paid && session.metadata?.appointmentId) {
            await supabase.from('appointments').update({ status: 'payment_submitted' }).eq('id', session.metadata.appointmentId)
